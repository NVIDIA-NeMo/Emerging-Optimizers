...
class TpRekls(opt_mixin.WeightDecayMixin, optim.Optimizer):
    ...
    recomputed via :func:`~emerging_optimizers.soap.soap_utils.get_eigenbasis_svd` from the kronecker
    factors. Each step calls svd twice — once on the pre-update L, R for the
    ...
    def step(self, closure: None = None) -> None:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # via svd since we do not persist eigenbases across steps.
                with utils.fp32_matmul_precision(self.fp32_matmul_prec):
                    pre_eigenbasis_list = soap_utils.get_eigenbasis_svd(kronecker_factor_list)
                ...
                with utils.fp32_matmul_precision(self.fp32_matmul_prec):
                    ...
                    # (matches update_eigenbasis_and_exp_avgs in soap.py for the eigh path; we use svd here).
                    state["exp_avg"] = soap.precondition(state["exp_avg"], pre_eigenbasis_list, dims=[[0], [1]])
                    eigenbasis_list = soap_utils.get_eigenbasis_svd(kronecker_factor_list)
                    state["exp_avg"] = soap.precondition(state["exp_avg"], eigenbasis_list, dims=[[0], [0]])
