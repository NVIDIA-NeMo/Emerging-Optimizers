...
`momentum=0.95`, `num_steps=11`, `zero_row_threshold=1e-3`, `eps=1e-20`, and `lr_correction=0.18`. The
optimizer expects the token or vocabulary dimension to be the first and larger matrix dimension.
Parameters must use BF16, FP16, or FP32; FP64 parameters are rejected explicitly.
